	...
	<table id="cart_summary" class="std">
		<thead>
			<tr>
				...
				<th class="cart_availability item">{l s='Reference.'}</th>
		...
		{foreach from=$products item=product name=productLoop}
			...
			{include file="./shopping-final.tpl"}
			...
			{if isset($customizedDatas.$productId.$productAttributeId)}
				...
				{if $product.quantity-$quantityDisplayed > 0}{include file="./shopping-final.tpl"}{/if}
			{/if}
		{/foreach}
		...
		{foreach $gift_products as $product}
			...
			{include file="./shopping-final.tpl" productLast=$product@last productFirst=$product@first}
		{/foreach}
		</tbody>
	{if count($discounts)}
		<tbody>
