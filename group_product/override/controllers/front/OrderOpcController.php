...
class OrderOpcControllerCore extends ParentOrderController
{
				...
				if (Tools::isSubmit('method'))
				{
					switch (Tools::getValue('method'))
					{
						...
						case 'cartReload':
							...
							$this->setTemplate(_PS_MODULE_DIR_.'group_product/shopping-cart.tpl');
							$this->display();
							break;
	...
	public function initContent()
	{
		...
		$this->context->smarty->assign(array(
			...
			'module_path'=>_PS_MODULE_DIR_ // here declare path for module tpl files
		));
		...
		$this->setTemplate(_PS_MODULE_DIR_.'group_product/order-opc.tpl');
		//$this->setTemplate(_PS_MODULE_DIR_.'group_product/shopping-cart.tpl');
	}
	...
	public function klarnaCheckout()
	{
		...
		$info=Db::getInstance()->getRow("select * from "._DB_PREFIX_."module_shop where `id_module` in (select id_module from "._DB_PREFIX_."module where name='klarnacheckout') and id_shop='".$id_shop."'");
			if(is_array($info) && $info['id_shop']==$id_shop)
			{
				$result=$this->context->smarty->assign(array('klarnabutton' => "enable"));
			}
			else
			{
				$result=$this->context->smarty->assign(array('klarnabutton' => "disable"));
			}			
		return $result;
	}	
}
